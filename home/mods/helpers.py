...
class LoggingContext(object):
    def __init__(self, logger, level=None, handlerd=None, close=True):
        self.logger = logger
        self.level = level
        self.handlerd = LOGGING['handlers'][handlerd].copy()
        self.handler = None
        self.close = close
    ...
    def __enter__(self):
        ...
        if self.handlerd:
            if isinstance(self.handlerd, dict):
              _ = self.handlerd.pop('class')
              f = self.handlerd['formatter']
              f2 = LOGGING['formatters'][f]['format']
              self.handlerd['formatter'] = f2
              self.handler = logging.FileHandler(**self.handlerd)
...
class BetterPrinter:
  ...
  def pframe(f):
    """
      on 'frames' doesnt work: str, format, repr
    ...
    """
    f_attrs = ['f_code', 'f_globals', 'f_lasti',
               'f_lineno', 'f_locals', 'f_restricted',
               'f_trace',]
    f_dict = {}
    for attr in f_attrs:
      if hasattr(f, attr):
        k = attr
        v = getattr(f, attr)
        f_dict.update({attr: (getattr(f, attr))})
    return f_dict
...
#
#       prev_filenamens.append(
#         f'{line_no}, len(pfn): {len(prev_filename)}, in internal_convert_to_tensor')
#
#       if line_no == 1092 and len(
#           opsl) > 1000:  # sum(['1092' in x[0] for x in opsl]) > 2:
#         opsl = []
#
#       lns = line_no_string = f"* {func_name:-<70}{line_no}"
...
def pmod(pkd=None):
  import os
  pkd = lpk()
  bp = '/Users/alberthan/PycharmProjects/magentaVAE/home/versions'
...
def jpy(fn, op):
  ...
  # bp: basepath
  # np: new path
  bp = '/Users/alberthan/PycharmProjects/magentaVAE/home/versions/data'
...
def rpictt(vl):
  ...
  # vbody
  vb = vl[1]
  vb = vb.replace('/Users/alberthan/PycharmProjects/magentaVAE/home/', '')
  # indent
  i = 2
  ic = i
