...
class PianorollSequence(events_lib.EventSequence):
    ...
    def _from_quantized_sequence(
        ...
        for note in quantized_sequence.notes:
            ...
            piano_roll[note_start_offset:note_end_offset,
                       note_pitch_offset] = 1

        events = [tuple(np.where(frame)[0]) for frame in piano_roll]
        np.set_printoptions(threshold=np.nan)
        mw.write(mw.model_dir, 'fq_pianoroll', piano_roll)
    ...
    def to_sequence(self,
        ...
        final_step = step + (len(open_notes) >
                             0)  # pylint: disable=g-explicit-length-test
...
def extract_pianoroll_sequences(
    ...
    pianoroll_seq = PianorollSequence(quantized_sequence=quantized_sequence,
                                      ...
                                      mod_writer=mw)
    quantized_nade_ns = pianoroll_seq.to_sequence()
    quantized_nade_ns.filename = quantized_sequence.filename
    mw.write(mw.model_dir, 'quantized_nade_ns', quantized_nade_ns)
