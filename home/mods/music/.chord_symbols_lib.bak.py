...
from refs.key_signatures import PITCH_NUM_TO_NOTE_LETTER as PNNL
...
def pnnl(key, midi_pitch_numbers):
  if type(midi_pitch_numbers) == int:
    return str(midi_pitch_numbers) + '(' + PNNL[key][midi_pitch_numbers%12] + ')'
  else:
    return {(number, PNNL[key][number%12]) for number in midi_pitch_numbers}
...
# d = OrderedDict(sorted(c.items(), key=lambda x: x[1]))
# pp = pprint.PrettyPrinter()
# wr('_CHORD_KINDS_BY_ABBREV', pp.pformat(d))
...
# changed
def _largest_chord_kind_from_relative_pitches(relative_pitches, path='', idx1=0):
  ...
  header_sd = idx1 + 'scale_degrees'
  wr(path, scale_degrees, (True, header_sd))
  best_chord_abbrev = None
  best_degrees = []
  for idx2, degrees in enumerate(itertools.product(*scale_degrees)):
    ...
    idx2 = idxfix(idx2 + 1)
    header = idx1 + idx2 + 'degree_steps: %s' % chord_abbrev
    wr(path, degree_steps, (True, header))
  ...
  header_rt = idx1 + 'best_chord_abbrev, best_degrees:'
  wr(path, (best_chord_abbrev, best_degrees), (True, header_rt))
...
# changed
def pitches_to_chord_symbol(path='', pitches=None):
  path += 'pitches'
  if not pitches:
    return constants.NO_CHORD
  wr(path, pnnl('f', pitches))

  # Convert to pitch classes and dedupe.
  pitch_classes = set(pitch % 12 for pitch in pitches)
  wr(path, 
    pnnl('f', pitch_classes), 
    (True, 'pitch_classes_1'))
  ...
  output1 = {'best_root': best_root, 'root_str': root_str,
             'best_degrees': best_degrees, 'best_abbrev': best_abbrev}
  header1 = 'best_chord_degrees: %s' % best_chord_degrees
  wr(path, output1, (True, header1))
  ...
  wr(path, modifications_str, (True, 'modifications_str'))
  if bass == best_root:
    wr(path, '%s%s%s' % (root_str, kind_str, modifications_str), 'final')
    return '%s%s%s' % (root_str, kind_str, modifications_str)
  else:
    bass_str = _pitch_class_to_string(*_transpose_pitch_class('C', 0, bass))
    wr(path, '%s%s%s/%s' % (root_str, kind_str, modifications_str, bass_str), 'final')
