...
def infer_chords_for_sequence(path='',
    sequence=None, instrument=None, min_notes_per_chord=3):
  ...
  wr(os.path.join(path, 'sorted_notes'), dict((k, v) for k, v in enumerate(sorted_notes))) # wr
  ...
  wr(os.path.join(path, 'events'), events) # wr
  ...
  for fn_idx, (time, idx, is_offset) in enumerate(events):
    #print()
    #print(fn_idx)
    #print(time, idx, is_offset)
    if time > current_time:
      fn_idx = idxfix3(fn_idx + 1)
      new_path = os.path.join(path, fn_idx)
      #print(new_path)
      active_pitches = set(sorted_notes[idx].pitch for idx in active_notes)
      #print(fn_idx)
      #print(active_pitches)
      if len(active_pitches) >= min_notes_per_chord:
        # Infer a chord symbol for the active pitches.
        figure = chord_symbols_lib.pitches_to_chord_symbol(new_path, active_pitches)
