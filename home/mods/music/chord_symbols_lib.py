...
from music21.harmony import CHORD_TYPES as m21
...
mag = _CHORD_KINDS
m21v = [v[0] for v in m21.values()]
m21v = [v.replace('-', 'b').split(',') for v in m21v]
magv = [x[1] for x in mag]
magv_bool = [v in m21v for v in magv]
m21v_bool = [v in magv for v in m21v]
#m21 = tuple(m21.items())
m21 = [((k,m21v[i]),m21v_bool[i]) for i,(k,v) in enumerate(m21.items())]
mag = list(zip(mag, magv_bool))

Chord = collections.namedtuple('Chord', 'intersection discard')
def split_lists(ls):
  chords = []
  for l in ls:
    chord = Chord(intersection=[], discard=[])
    for elm in l:
      if elm[-1]:
        chord.intersection.append(elm)
      else:
        chord.discard.append(elm)
    chords.append(chord)

  return chords

m21,mag = split_lists([m21,mag])
assert len(m21.intersection) == len(mag.intersection), "intersections not equal"
m21i = sorted([m for m in m21.intersection], key=lambda x: x[0][1])
magi = sorted([m for m in mag.intersection], key=lambda x: x[0][1])
intersections = [magi[i][0] for i in range(len(m21i)) if m21i[i][0][1]==magi[i][0][1]]
discard = m21.discard + mag.discard
_CHORD_KINDS = intersections
...
def pitches_to_chord_symbol(pitches):
  ...
  for root in pitch_classes:
    ...
    print(f'relative_pitches: {relative_pitches}')
    abbrev, degrees = _largest_chord_kind_from_relative_pitches(
        relative_pitches)
    print(f'abbrev, degrees: {abbrev, degrees}')
    if abbrev is not None:
      if best_abbrev is None or (
          len(_CHORD_KINDS_BY_ABBREV[abbrev]) >
          len(_CHORD_KINDS_BY_ABBREV[best_abbrev])):
        best_root = root
        print(f'best_root: {best_root}')
        best_abbrev = abbrev
        print(f'best_abbrev: {best_abbrev}')
        best_degrees = degrees
        print(f'best_degrees: {best_degrees}')

  if best_root is None or best_abbrev == 'ped':
